...
const InvoiceDetailsModal: React.FC<InvoiceDetailsModalProps> = ({ invoice, companyProfile, onClose, onUpdateInvoice }) => {
    ...
    const handleRecordPayment = (e: React.FormEvent) => {
        ...
        if (payment.amount <= 0 || payment.amount > balanceDue + 0.001) { // Add tolerance for float issues
            toast.error(`Payment must be between $0.01 and ${formatCurrency(balanceDue)}.`);
            return;
        }
                    ...
                    <div className="invoice-preview p-6 md:p-8 bg-white text-gray-900 font-sans text-sm border border-gray-200 rounded-lg">
                         ...
                         <section className="client-details grid grid-cols-1 md:grid-cols-2 gap-4 py-6">
                            ...
                            <div className="text-left md:text-right mt-4 md:mt-0">
                                <p><span className="font-semibold text-slate-600">Issue Date:</span> {formatDate(invoice.issueDate)}</p>
                                <p><span className="font-semibold text-slate-600">Due Date:</span> {formatDate(invoice.dueDate)}</p>
                            </div>
                        </section>
                            ...
                            <table className="invoice-items w-full text-left min-w-[500px]">
                                <thead>
                                    <tr className="bg-slate-50">
                                        <th className="p-3 font-semibold text-slate-600 uppercase">Description</th>
                                        <th className="p-3 font-semibold text-slate-600 uppercase text-center">Qty</th>
                                        <th className="p-3 font-semibold text-slate-600 uppercase text-right">Unit Price</th>
                        ...
                        <footer className="invoice-footer flex justify-end pt-6">
                            <div className="w-full max-w-sm">
                                <div className="flex justify-between py-2 text-slate-600"><span>Subtotal:</span><span>{formatCurrency(invoice.total)}</span></div>
                                <div className="flex justify-between py-2 text-slate-600"><span>Paid:</span><span>-{formatCurrency(invoice.amountPaid)}</span></div>
                                <div className="flex justify-between py-3 font-bold text-lg text-slate-800 border-t mt-2"><span>Balance Due:</span><span>{formatCurrency(balanceDue)}</span></div>
                            </div>
                        </footer>
                    </div>
                    {/* Payment Section */}
                    <div className="mt-8">
                        <h4 className="text-lg font-semibold mb-4 text-slate-800 dark:text-slate-200">Payment History</h4>
                        {invoice.paymentRecords.length > 0 ? (
                            ...
                            <p className="text-sm text-slate-500">No payments have been recorded for this invoice.</p>
                        )}
                        ...
                        {isPaymentFormVisible && (
                            <form onSubmit={handleRecordPayment} className="mt-6 p-4 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                                <h5 className="text-md font-semibold mb-4">Add Payment</h5>
                                ...
                                <div className="mt-4 flex justify-end space-x-2">
                                    <button type="button" onClick={() => setIsPaymentFormVisible(false)} className="px-3 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm">Cancel</button>
                                    <button type="submit" className="px-3 py-2 text-sm font-medium text-white bg-sky-600 border border-transparent rounded-md shadow-sm hover:bg-sky-700">Save Payment</button>
                                </div>
                            </form>
                        )}
                    </div>
