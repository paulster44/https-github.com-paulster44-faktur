...
const InvoiceList: React.FC<InvoicesPageProps> = ({ invoices, onNavigate }) => {
     return (
        <>
        {invoices.length === 0 ? (
...
const InvoicesPage: React.FC<InvoicesPageProps> = ({ invoices, onNavigate }) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-lg">
            ...
            <div className="p-4 md:p-6">
                {activeTab === 'list' && <InvoiceList invoices={invoices} onNavigate={onNavigate} />}
                {activeTab === 'design' && <InvoiceDesignStudio />}
            </div>
        </div>
    );
};
